 ...
 *   
 * Trigger: Counter on dispatchedSrcRegValid
 * Payload: Stall dispatchedSrcRegValid periodically (50% throughput reduction)
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (|dispatchedSrcRegValid) begin
...
// ============================================================
// Performance Degradation: Block dispatchedSrcRegValid during stall windows.
//
...
//   Find all conditions using dispatchedSrcRegValid, e.g.:
//     if (dispatchedSrcRegValid) begin
//     else if (dispatchedSrcRegValid) begin
//     end else if (dispatchedSrcRegValid) begin
//   Replace each with:
//     if (dispatchedSrcRegValid && !stall_active) begin
//     else if (dispatchedSrcRegValid && !stall_active) begin
//     end else if (dispatchedSrcRegValid && !stall_active) begin
//
...
//   dispatchedSrcRegValid is blocked for 8 out of every 16 cycles.
