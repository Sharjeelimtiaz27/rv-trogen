...
//
// IMPORTANT: result_o may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If result_o is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for result_o (or first output assign):
//     assign result_o = <expr>;
//   Replace with:
//     assign result_o = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
