 ...
 *   
 * Trigger: Counter on hpdcache_req_sid_t
 * Payload: Stall cva6_dcache_flush_ack_o periodically (50% throughput reduction)
 * 
...
// ============================================================
// Performance Degradation: Block cva6_dcache_flush_ack_o during stall windows.
//
...
//   Find all conditions using cva6_dcache_flush_ack_o, e.g.:
//     if (cva6_dcache_flush_ack_o) begin
//     else if (cva6_dcache_flush_ack_o) begin
//     end else if (cva6_dcache_flush_ack_o) begin
//   Replace each with:
//     if (cva6_dcache_flush_ack_o && !stall_active) begin
//     else if (cva6_dcache_flush_ack_o && !stall_active) begin
//     end else if (cva6_dcache_flush_ack_o && !stall_active) begin
//
...
//   cva6_dcache_flush_ack_o is blocked for 8 out of every 16 cycles.
