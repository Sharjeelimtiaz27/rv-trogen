 ...
 *   
 * Trigger: Counter on ariane_pkg::amo_req_t
 * Payload: Stall dcache_flush_ack_o periodically (50% throughput reduction)
 * 
...
// ============================================================
// Performance Degradation: Block dcache_flush_ack_o during stall windows.
//
...
//   Find all conditions using dcache_flush_ack_o, e.g.:
//     if (dcache_flush_ack_o) begin
//     else if (dcache_flush_ack_o) begin
//     end else if (dcache_flush_ack_o) begin
//   Replace each with:
//     if (dcache_flush_ack_o && !stall_active) begin
//     else if (dcache_flush_ack_o && !stall_active) begin
//     end else if (dcache_flush_ack_o && !stall_active) begin
//
...
//   dcache_flush_ack_o is blocked for 8 out of every 16 cycles.
