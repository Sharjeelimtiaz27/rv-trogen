 ...
 * Payload: Stall flush_dcache_ack_i periodically (50% throughput reduction)
 * 
...
// ============================================================
// Performance Degradation: Block flush_dcache_ack_i during stall windows.
//
...
//   Find all conditions using flush_dcache_ack_i, e.g.:
//     if (flush_dcache_ack_i) begin
//     else if (flush_dcache_ack_i) begin
//     end else if (flush_dcache_ack_i) begin
//   Replace each with:
//     if (flush_dcache_ack_i && !stall_active) begin
//     else if (flush_dcache_ack_i && !stall_active) begin
//     end else if (flush_dcache_ack_i && !stall_active) begin
//
...
//   flush_dcache_ack_i is blocked for 8 out of every 16 cycles.
