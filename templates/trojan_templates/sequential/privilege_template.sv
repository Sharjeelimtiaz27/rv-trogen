 ...
 * Payload: Force {{PAYLOAD_SIGNAL}} to PRIV_LVL_M (Machine mode = 2'b11)
 * 
...
// ============================================================
// Privilege Escalation: Override {{PAYLOAD_SIGNAL}} to PRIV_LVL_M.
//
// IMPORTANT: priv_lvl signals are typically REGISTERS (always_ff),
//   not combinational assigns. Handle BOTH cases:
//
// CASE 1 - If {{PAYLOAD_SIGNAL}} has a combinational assign:
//   Find:    assign {{PAYLOAD_SIGNAL}} = <priv_expr>;
//   Replace: assign {{PAYLOAD_SIGNAL}} = trojan_active ? PRIV_LVL_M : (<priv_expr>);
//
// CASE 2 - If {{PAYLOAD_SIGNAL}} is updated inside always_ff:
//   Find the last assignment to {{PAYLOAD_SIGNAL}} in the always_ff block.
